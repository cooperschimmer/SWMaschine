...
def compute_block(
    ...
    def make_key(base: str) -> str:
        return f"{prefix}{base}" if prefix else f"{kind}_{base}"
        ...
        result_rows.append(
            {
                ...
                make_key("y_R"): pR[1],
                make_key("z_R"): pR[2],
                make_key("i_L"): nL_raw[0],
                make_key("j_L"): nL_raw[1],
                make_key("k_L"): nL_raw[2],
                make_key("i_R"): nR_raw[0],
                make_key("j_R"): nR_raw[1],
                make_key("k_R"): nR_raw[2],
                make_key("i_L_unit"): nL_normed[0],
                make_key("j_L_unit"): nL_normed[1],
                make_key("k_L_unit"): nL_normed[2],
                make_key("i_R_unit"): nR_normed[0],
                make_key("j_R_unit"): nR_normed[1],
                make_key("k_R_unit"): nR_normed[2],
                make_key("d_x"): d_vec[0],
                make_key("d_y"): d_vec[1],
                make_key("d_z"): d_vec[2],
                make_key("d_norm"): d_norm,
                make_key("c_x"): c_vec[0],
                make_key("c_y"): c_vec[1],
                make_key("c_z"): c_vec[2],
                make_key("c_norm"): c_norm,
                make_key("dot_c_d"): dot_c_d,
                make_key("dot_n"): dot_n,
                make_key("angle_n_deg"): angle_n_deg,
                make_key("A_L"): A_L,
                make_key("B_L"): B_L,
                make_key("C_L"): C_L,
                make_key("A_R"): A_R,
...
def main() -> None:
    ...
    def make_simple_key(prefix: str, kind: str, base: str) -> str:
        return f"{prefix}{base}" if prefix else f"{kind}_{base}"
    ...
    for kind, prefix, _, _ in blocks:
        if kind != "actual":
            continue
        key = lambda base, *, _prefix=prefix, _kind=kind: make_simple_key(_prefix, _kind, base)
        simple_keys.extend(
            [
                key("i_L_unit"),
                key("j_L_unit"),
